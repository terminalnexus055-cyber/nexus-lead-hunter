          ...
          {results && (
            <>
              ...
              <div style={s.section}>
                ...
                <div style={s.row}>
                  <span style={s.key}>Groq API key</span>
                  <span>
                    <StatusPill ok={results.env?.groqKeySet} label={results.env?.groqKeySet ? `Set (${results.env.groqKeyPrefix}, ${results.env.groqKeyLength} chars)` : 'NOT SET in Vercel env vars'} />
                  </span>
                </div>
                <div style={s.row}>
              ...
              {/* GROK MODELS */}
              <div style={s.section}>
                <div style={s.sLabel}>Groq — available models</div>
                {results.groqModels?.ok ? (
                  <>
                    <div style={{ ...s.row, ...s.ok }}>✓ Connected to xAI API</div>
                    <div style={s.row}>
                      <span style={s.key}>Models available</span>
                      <span style={s.val}>
                        {results.groqModels.body?.data?.map(m => m.id).join(', ') || 'none returned'}
                      </span>
                    </div>
                  </>
                ) : (
                  <>
                    <div style={{ ...s.row, ...s.err }}>
                      ✗ Failed — HTTP {results.groqModels?.status}
                    </div>
                    <div style={s.pre}>{JSON.stringify(results.groqModels?.body, null, 2)}</div>
                  </>
                )}
              </div>
              ...
              {/* GROK TEST CALL */}
              <div style={s.section}>
                <div style={s.sLabel}>Groq — test completion call</div>
                {results.groqTestCall?.ok ? (
                  <div style={{ ...s.row, ...s.ok }}>
                    ✓ Works — using model: <strong style={{ marginLeft: 6 }}>{results.groqTestCall.modelUsed}</strong>
                  </div>
                ) : (
                  <>
                    <div style={{ ...s.row, ...s.err }}>
                      ✗ Failed — HTTP {results.groqTestCall?.status} using model "{results.groqTestCall?.modelUsed}"
                    </div>
                    <div style={s.pre}>{results.groqTestCall?.rawBody || results.groqTestCall?.error}</div>
                    {results.groqTestCall?.availableModels?.length > 0 && (
                      <div style={{ ...s.row, ...s.warn, marginTop: 8 }}>
                        ⚠ Available models on your account: {results.groqTestCall.availableModels.join(', ')}
                      </div>
                    )}
                  </>
                )}
